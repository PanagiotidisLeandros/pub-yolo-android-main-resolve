...
import org.tensorflow.lite.examples.objectdetection.MetaData
...
import org.tensorflow.lite.examples.objectdetection.Output0
...
class InstanceSegmentation(
    ...
    fun invoke(frame: Bitmap) {
        val inputArray = preProcess(frame)
        val outputMap = runInference(inputArray)
        
        val coordinatesBuffer = outputMap[0] as ByteBuffer
        val maskProtoBuffer = outputMap[1] as ByteBuffer
        
        val bestBoxes = bestBox(coordinatesBuffer) ?: run {
            instanceSegmentationListener.onEmpty()
            return
        }

        val maskProto = reshapeMaskOutput(maskProtoBuffer)

        val segmentationResults = bestBoxes.map {
            SegmentationResult(
                box = it,
                mask = getFinalMask(INPUT_SIZE, INPUT_SIZE, it, maskProto)
            )
        }

        instanceSegmentationListener.onDetect(
            interfaceTime = 0, // You might want to measure this
            results = segmentationResults,
            preProcessTime = 0, // You might want to measure this
            postProcessTime = 0 // You might want to measure this
        )
    }
    ...
    private fun reshapeMaskOutput(buffer: ByteBuffer): List<Array<FloatArray>> {
        buffer.rewind()
        ...
        return List(masksNum) { mask ->
            Array(xPoints) { r ->
                FloatArray(yPoints) { c ->
                    floatBuffer.get(masksNum * yPoints * r + masksNum * c + mask)
                }
            }
        }
    }
    ...
    private fun bestBox(buffer: ByteBuffer): List<Output0>? {
        buffer.rewind()
        val floatBuffer = buffer.asFloatBuffer()
        ...
        for (c in 0 until numElements) {
            ...
            while (currentInd < (numChannel - masksNum)){
                val value = floatBuffer.get(arrayIdx)
                if (value > maxConf) {
                    maxConf = value
                    maxIdx = currentInd - 4
                }
            ...
            if (maxConf > CONFIDENCE_THRESHOLD) {
                ...
                val cx = floatBuffer.get(c) // 0
                val cy = floatBuffer.get(c + numElements) // 1
                val w = floatBuffer.get(c + numElements * 2)
                val h = floatBuffer.get(c + numElements * 3)
                val x1 = cx - (w/2F)
                val y1 = cy - (h/2F)
                val x2 = cx + (w/2F)
                val y2 = cy + (h/2F)
                ...
                while (currentInd < numChannel){
                    maskWeight.add(floatBuffer.get(arrayIdx))
                    currentInd++
                    arrayIdx += numElements
                }
    ...
    companion object {
        private const val INPUT_SIZE = 640 // or whatever size your model expects
    ...
    private fun runInference(inputArray: Array<ByteBuffer>): Map<Int, Any> {
        val outputMap = HashMap<Int, Any>()
        val outputBuffer0 = TensorBuffer.createFixedSize(intArrayOf(1, 116, 8400), DataType.FLOAT32)
        val outputBuffer1 = TensorBuffer.createFixedSize(intArrayOf(1, 32, 160, 160), DataType.FLOAT32)
        outputMap[0] = outputBuffer0.buffer.rewind()
        outputMap[1] = outputBuffer1.buffer.rewind()
